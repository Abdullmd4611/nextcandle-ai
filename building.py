...
def _normalise_index(df):
    ...
    Numeric timestamps are automatically detected as
    ...
    # --------------------------------------------------------
    # CASE 1:
    # Data already has a DatetimeIndex
    # --------------------------------------------------------
    ...
    if time_column is None:
        ...
        raise ValueError(
            "Market data has neither a DatetimeIndex "
            "nor a recognized timestamp column. "
    ...
    if pd.api.types.is_numeric_dtype(values):
        ...
        if len(non_null_values) == 0:
            ...
            raise ValueError(
                f"Timestamp column '{time_column}' contains "
                "no usable values."
            )
...
def build_current_candle_target(
    ...
    if neutral_threshold <= 0:
        ...
        raise ValueError(
            "neutral_threshold must be greater than zero."
        )
    ...
    target.loc[
        candle_return < -neutral_threshold
    ] = BEARISH
...
def build_current_structure_target(
    df_15m
):
    """
...
def _build_open_cross_features(
    ...
    Lower timeframe information is shifted so that only
    completed lower-timeframe candles are available at the
    target 15M start.
    """
    ...
    if (
        "m1_close" in result.columns
        and
        "m5_close" in result.columns
    ):
...
def prepare_open_training_data(
    ...
    # ========================================================
    # BUILD SAME-CANDLE DIRECTION TARGET
    # ========================================================
    ...
    # ========================================================
    # BUILD SAME-CANDLE STRUCTURE TARGET
    # ========================================================

    structure_target = (
        build_current_structure_target(
            df_15m=df_15m
        )
    )
